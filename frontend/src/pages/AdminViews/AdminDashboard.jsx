...
export function AdminDashboard() {
    ...
    stompClient.connect({}, () => {
      ...
      stompClient.subscribe("/topic/stats-updates", (message) => {
        try {
          ...
          console.log("Received stats update:", data); // Debug log
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        console.log("Fetching data for date:", today); // Debug log
        ...
        console.log("Daily stats response:", dailyRes.data); // Debug log

        // Handle responses with better error checking
        setStatsData({
          todaysRevenue: Number(dailyRes.data?.todaysRevenue) || 0,
          totalOrders: Number(dailyRes.data?.totalOrders) || 0,
          avgOrderValue: Number(dailyRes.data?.avgOrderValue) || 0,
          avgPreparationTime: Number(dailyRes.data?.avgPreparationTime) || 0,
        });
  ...
  console.log("Current stats data:", statsData); // Debug log

  const stats = [
    {
      title: "Today's Revenue",
      value: `$${formatCurrency(statsData.todaysRevenue)}`,
    ...
    },
    {
      ...
      value: `${formatCurrency(statsData.avgPreparationTime)} min`,
  ...
  const handleExport = (format) => {
    ...
    if (format === "json") {
      ...
      csvContent += `Today's Revenue,$${formatCurrency(
        statsData.todaysRevenue
      )}\n`;
      csvContent += `Total Orders,${statsData.totalOrders}\n`;
      csvContent += `Avg. Order Value,$${formatCurrency(
        statsData.avgOrderValue
      )}\n`;
      csvContent += `Avg. Preparation Time,${formatCurrency(
        statsData.avgPreparationTime
      )} min\n\n`;
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex space-x-3">
        <div className="relative">
          ...
          {showExportDropdown && (
            <div
              className="absolute bg-white shadow-lg rounded-md mt-1 z-10 w-40"
      ...
      {/* Charts and Data - Only Top Selling and Sales by Category */}
